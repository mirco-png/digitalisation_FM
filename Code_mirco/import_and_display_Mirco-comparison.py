...
for i in df["Vorname"]:
    ...
    while x < 15:
        ...
        if age > 18:
            xc +=1
        stringtm = f"{2008+x+1} "+ "Market value"
        stringpm = f"{2008+x} "+ "Market value"
        tm = df[stringtm][c] /1000
        pm = int(df[stringpm][c] * ls_relevant_d[23+xc+1]/ls_relevant_d[23+xc]) /1000
        dif = tm - pm
        if tm !=0:
            difp = abs(dif)*100/tm
        else:
    ...
    while x3 < 5:
        ...
        if age > 18:
            xs +=3
        string1tm = f"{2008+x2+1} "+ "Market value"
        string2tm = f"{2008+x2+2} "+ "Market value"
        string3tm = f"{2008+x2+3} "+ "Market value"
        string1pm = f"{2008+x2} "+ "Market value"
        tm1 = df[string1tm][c] /1000
        tm2 = df[string2tm][c] /1000
        tm3 = df[string3tm][c] /1000
        pm1 = int(df[string1pm][c] * ls_relevant_d[23+xs+1]/ls_relevant_d[23+xs])
        pm2 = int(pm1 * ls_relevant_d[23+xs+2]/ls_relevant_d[23+xs+1])
        pm3 = int(pm2 * ls_relevant_d[23+xs+3]/ls_relevant_d[23+xs+2])
        dif1 = tm1 - pm1
        dif2 = tm2 - pm2
        dif3 = tm3 - pm3
...
for a, t, p, i, p3, i3, pp in zip(ls_age_t, ls_tm_t, ls_pm_t, ls_dif_t, ls_pm3_t, ls_dif3_t, ls_difp_t):
    ...
    n = df["Nachname"][co]
    d[f"age for {v} {n}"] = a
    d[f"deviation market value for {v} {n} (one-year prognostic)"] =      i
    d[f"deviation market value in % for {v} {n}"] = pp
    d[f"deviation market value for {v} {n} (three-year prognostic)"] =  i3
    d[f"predicted market value for {v} {n} (three-year prognostic)"] =  p3
    d[f"predicted market value for {v} {n} (one-year prognostic)"] =      p
...
for n, v in zip(df["Vorname"], df["Nachname"]):
    df_new.plot(y=[f"predicted market value for {n} {v} (one-year prognostic)", f"transfermarkt market value for {n} {v}", f"predicted market value for {n} {v} (three-year prognostic)", ], x=f"age for {n} {v}" , figsize=(14, 7))
    ...
    df_new.plot(y=[f"deviation market value for {n} {v} (one-year prognostic)", f"deviation market value for {n} {v} (three-year prognostic)"], x=f"age for {n} {v}" , figsize=(14, 7))
    plt.title(f"deviation market value and forecaste error of {ls_t[c]}% for {n} {v}")
    plt.xlabel("age", 
            family='serif', 
            color='r', 
            weight='normal', 
    ...
    df_new.plot(y=[f"predicted market value for {n} {v} (three-year prognostic)", f"transfermarkt market value for {n} {v}"], x=f"age for {n} {v}" , figsize=(14, 7))
    ...
    df_new.plot(y=[f"deviation market value for {n} {v} (three-year prognostic)"], x=f"age for {n} {v}" , figsize=(14, 7))
    plt.xlabel("age", 
            family='serif', 
            color='r', 
            weight='normal', 
